...
import { uploadPhoto } from './src/photo.js';
...
function setupSettings() {
  ...
  const darkModal = new Modal('Settings', settings, settings.querySelector('.modal-content'));
  darkModal.render();
}
...
function setupAddText() {
  ...
  const textModal = new Modal('Add some text', textInputs, textInputs.querySelector('.modal-content'));
  textModal.render();
}

async function setupUploadPhoto() {
  const fileInput = document.getElementById('upload-photo');
  const uploadPhotoBtn = document.getElementById('upload-save');
  const previewCanvas = document.getElementById('upload-preview');

  const photo = await uploadPhoto(fileInput, previewCanvas);

  uploadPhotoBtn.addEventListener('click', e => {
    uploadPhoto(fileInput, textLayer);
    redrawMeme();
  });

  const uploadModal = new Modal('Upload a photo', fileInput, fileInput.querySelector('.modal-content'));
  uploadModal.render();
}
...
async function setupTakeSelfie() {
  ...
  const selfieModal = new Modal('Take a selfie', selfie, selfie.querySelector('.modal-content'));
  selfieModal.render();
